<div class="md:block hidden w-full leading-0">
    <button class="text-slate-800 dark:text-white focus:ring-0 focus:outline-none font-medium rounded-lg text-sm text-center
        ...
        <div class="lg:h-8 lg:w-8 h-7 w-7 rounded-full flex-1 ltr:mr-[10px] rtl:ml-[10px]">
            <img class="block w-full h-full object-cover rounded-full" src="<?php echo e(auth()->user()->getFirstMediaUrl('profile-image', 'preview') ?:
        ...
        <div class="ltr:text-left rtl:text-right">
            ...
            <!-- <small class="text-[9px] block"><?php echo e(auth()->user()->roles()->first()?->name); ?></small> -->
        </div>
    ...
    <div class="dropdown-menu z-10 hidden bg-white divide-y divide-slate-100 shadow w-44 dark:bg-slate-800 border dark:border-slate-700 !top-[23px] rounded-md
        ...
        <ul class="py-1 text-sm text-slate-800 dark:text-slate-200" :class="listView ? 'z-20 opacity-100 top-[61px]' : 'opacity-0 -z-20 top-5' " x-show="listView" @click.away="listView = false">
            <li>
                <a href="<?php echo e(route('profiles.index')); ?>" class="flex items-center px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white font-inter text-sm text-slate-600
                      dark:text-white font-normal" class="<?php echo \Illuminate\Support\Arr::toCssClasses(['country-list', 'active'=>
            ...
            <li>
                <a href="<?php echo e(route('general-settings.show')); ?>" class="flex items-center px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white font-inter text-sm text-slate-600
                      dark:text-white font-normal" class="<?php echo \Illuminate\Support\Arr::toCssClasses(['country-list', 'active'=>
            ...
            <li>
                <form method="POST" action="<?php echo e(route('logout')); ?>" class="flex items-center px-4 py-2 hover:bg-slate-100 dark:hover:bg-slate-600 dark:hover:text-white font-inter text-sm text-slate-600
...
<?php /**PATH D:\Project\dashcode-laravel-full-source-code\resources\views/components/nav-user-dropdown.blade.php ENDPATH**/ ?>